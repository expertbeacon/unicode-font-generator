    ...
    return (
      <div className="mb-5 pb-4 border-b leading-7 flex flex-col lg:flex-row lg:items-center justify-start gap-5">
        ...
        <div className="relative flex flex-col lg:flex-row lg:justify-between w-full lg:items-center gap-3">
          <div className="text-2xl md:text-3xl font-bold p-4 bg-gradient-to-r from-green-50 to-blue-50 dark:from-green-950/20 dark:to-blue-950/20 rounded-lg border-2 border-green-200 dark:border-green-800 shadow-lg">
            {transformedContent}
          </div>
          <div className="flex items-center gap-3">
            <Copy className="text-sm h-8 rounded-lg">{transformedContent}</Copy>
            <Button className="h-8 rounded-lg" variant="outline" size="icon"><a target="_blank" href={`https://x.com/intent/post?text=${encodeURIComponent(transformedContent)}&utm_source=${appConfig.appDomain}`}>𝕏</a></Button>
          </div>
        </div>
      </div>
    );
